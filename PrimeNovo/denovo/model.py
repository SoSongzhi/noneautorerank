...
class Spec2Pep(pl.LightningModule, ModelMixin):
    ...
    **kwargs : Dict
    ...
    def forward(
        ...
        enc_out, enc_mask = self.encoder(spectra)
        # print("\n" + "="*80)
        # print("[Matrix Shape Info]")
        # print("="*80)
        # print(f"1. Encoder output (enc_out) shape: {enc_out.shape}")
        # print(f"   - Batch size: {enc_out.shape[0]}")
        # print(f"   - Sequence length (peaks): {enc_out.shape[1]}")
        # print(f"   - Model dimension (dim_model): {enc_out.shape[2]}")
        # print(f"\n   Statistics:")
        # print(f"   - Min: {enc_out.min().item():.6f}")
        # print(f"   - Max: {enc_out.max().item():.6f}")
        # print(f"   - Mean: {enc_out.mean().item():.6f}")
        # print(f"   - Std: {enc_out.std().item():.6f}")
        # print(f"\n   First 5 timesteps, first 10 dimensions:")
        # print(enc_out[0, :5, :10].cpu().detach().numpy())
        
        output_logits, _, output_list = self.decoder(None, precursors, enc_out, enc_mask)
        # print(f"\n2. Decoder logits shape: {output_logits.shape}")
        # print(f"   - Batch size: {output_logits.shape[0]}")
        # print(f"   - Max sequence length: {output_logits.shape[1]}")
        # print(f"   - Vocabulary size: {output_logits.shape[2]}")
        # print(f"\n   Statistics:")
        # print(f"   - Min: {output_logits.min().item():.6f}")
        # print(f"   - Max: {output_logits.max().item():.6f}")
        # print(f"   - Mean: {output_logits.mean().item():.6f}")
        # print(f"   - Std: {output_logits.std().item():.6f}")
        
        # probs = torch.nn.functional.softmax(output_logits, dim=-1)
        # print(f"\n3. Softmax probability matrix shape: {probs.shape}")
        # print(f"   Statistics:")
        # print(f"   - Min: {probs.min().item():.6f}")
        # print(f"   - Max: {probs.max().item():.6f}")
        # print(f"   - Mean: {probs.mean().item():.6f}")
        
        # print(f"\n   Top-5 probabilities for first 3 timesteps:")
        # for t in range(min(3, probs.shape[1])):
        #     top_probs, top_indices = torch.topk(probs[0, t], 5)
        #     print(f"   Timestep {t}:")
