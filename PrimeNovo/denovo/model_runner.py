...
def _execute_existing(
    ...
    class MyDirectory:
        ...
        def cleanup(self):
            """Cleanup method for compatibility with TemporaryDirectory"""
            pass  # Custom temp dir is not automatically cleaned up
    if not config["temp_dir_auto"]: # if we don't use auto-generated temp dir
        # Use custom temp directory from config
        temp_path = config.get("temp_dir_path", "D:/PrimeNovo_temp")
        os.makedirs(temp_path, exist_ok=True)
        tmp_dir = MyDirectory(temp_path)
    else:
