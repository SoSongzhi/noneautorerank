...
"""
批处理版本的MGF文件测试脚本
支持在单GPU上一次处理多个spectrum以加速
完全兼容原版逻辑，只是增加了batch processing

使用方法:
python test_any_mgf_batch.py <mgf_file_path> [--batch-size N]
python test_any_mgf_batch.py <mgf_file_path> --analyze  # 分析peptide分布

示例:
python test_any_mgf_batch.py "C:\\Users\\research\\Desktop\\Alldata\\high9-massfilter\\high.mouse.PXD004948_mass_filtered.mgf" --batch-size 8
python test_any_mgf_batch.py "testdata/high_nine_validation_1000_converted.mgf" --num_spectra 100 --batch-size 4
python test_any_mgf_batch.py "data.mgf" --analyze  # 只分析peptide分布
"""
...
示例:
  ...
  python test_any_mgf.py "data.mgf" --analyze  # 只分析peptide分布
    ...
    parser.add_argument('--batch-size', type=int, default=8,
                       help='批处理大小（默认：8，设置>1可加速但需要更多内存）')
    ...
    logger.info(f"\n{'='*80}")
    logger.info("PiPrime + HighNine Reranker - Batch Processing Mode")
    logger.info(f"{'='*80}")
    ...
    for batch_start in range(0, total_spectra, batch_size):
        ...
        batch_spectra = spectra_list[batch_start:batch_end]
        
        logger.info(f"Processing batch {batch_start//batch_size + 1}/{(total_spectra + batch_size - 1)//batch_size} "
                   f"(spectra {batch_start}-{batch_end-1})")
        
        for batch_idx, spec in enumerate(batch_spectra):
            ...
            try:
                ...
                logger.error(f"Error processing spectrum {idx}: {e}")
                import traceback
                traceback.print_exc()
                continue
